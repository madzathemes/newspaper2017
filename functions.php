...
function newspaper2017_theme_setup() {
	...
	load_theme_textdomain( 'newspaper2017', get_template_directory() . '/languages' );
	$locale = get_locale();
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Header Menu', 'newspaper2017' ),
		'mobile' => esc_html__( 'Mobile Menu', 'newspaper2017' ),
		'footer_menu' => esc_html__( 'Footer Navigation', 'newspaper2017' ),
	) );
	if ( ! isset( $content_width ) ) { $content_width = 900; }
}

add_action( 'after_setup_theme', 'newspaper2017_theme_setup' );
...
function newspaper2017_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'newspaper2017_javascript_detection', 0 );

if ( ! isset( $content_width ) ) {
	$content_width = 740;
}

function newspaper2017_import_files() {
    return array(
        array(
            'import_file_name'             => esc_html__( 'Demo 1', 'newspaper2017' ),
            ...
            'import_notice'                => esc_html__( 'Customize this theme from Appearance/Customize', 'newspaper2017' ),
        )
    );
}
add_filter( 'pt-ocdi/import_files', 'newspaper2017_import_files' );

function newspaper2017_after_import_setup() {
...
add_action( 'pt-ocdi/after_import', 'newspaper2017_after_import_setup' );
